...
namespace EVPStrengthModel
{
  ...
  //    chg_basis_4(cc66.pointer(), cc3333.pointer(), 4, 6, B_basis.pointer());
    ...
    // note, dont use strength_state_vars update device or host is not encouranged
    // this could lead to data been overwritten if multiple strength model are used 
    // within the same simulation of different elements

    // write data to device side of strength_state_vars
    FOR_ALL(elem_gid, 0, num_elems, {
      ...
      if (material(mat_id).strength_model == STRENGTH_MODEL::evp) {
        for(size_t var=0; var<num_strength_state_vars; var++){
          strength_state_vars(elem_gid,var) = state_vars(elem_gid,var);
        }
      }
    });

    // write data to host side of strength_state_vars
    for (size_t elem_gid = 0; elem_gid < num_elems; elem_gid++) {
      size_t mat_id = elem_mat_id.host(elem_gid);
      size_t num_strength_state_vars = material.host(mat_id).num_strength_state_vars;
      // only fill the state_vars of elements that use this model
      if (material.host(mat_id).strength_model == STRENGTH_MODEL::evp) {
        for(size_t var=0; var<num_strength_state_vars; var++){
          strength_state_vars.host(elem_gid,var) = state_vars.host(elem_gid,var);
        }
      }
    }
    ...
    const double time)
  {
    ...
    // Decompress strength_state_vars (this may become its own function later)
    // -----------------------------------------------------------------------------
    // read basis change matrix
    ViewCMatrixKokkos <double> B_basis(&strength_state_vars(elem_gid,260),3,3,6);

    ViewCMatrixKokkos <double> cc66(&strength_state_vars(elem_gid, 0), 6, 6); // crystal elastic stiffness 0-35
    ...
    int nsmx = strength_state_vars(elem_gid, 36);
    //if (nsmx == 12){
      const int nsm = 12*2;
    //}
    real_t nrs = strength_state_vars(elem_gid, 37);
    real_t gamd0 = strength_state_vars(elem_gid, 38);
    ViewCMatrixKokkos <double> voceParam(&strength_state_vars(elem_gid, 39),5);
    real_t hself = strength_state_vars(elem_gid, 44);
    real_t hlate = strength_state_vars(elem_gid, 45);
    ...
    // ViewFMatrixKokkos <double> schca(&strength_state_vars(elem_gid,46), 5, nsm); // crystal plastic Schmid tensors 46-105
    ViewFMatrixKokkos <double> dnca(&strength_state_vars(elem_gid,46), 3, nsm); // crystal slip normal vectors 46-81
    ViewFMatrixKokkos <double> dbca(&strength_state_vars(elem_gid,118), 3, nsm); // crystal slip normal vectors 46-81
    ...
    ViewCMatrixKokkos <double> crss(&strength_state_vars(elem_gid,190), 2, nsm); // forward and backward critical resolved shear stress 118-141

    ViewCMatrixKokkos <double> strain_n(&strength_state_vars(elem_gid,238), 3, 3); // strain at previous timestep 142-150
    ViewCMatrixKokkos <double> ept_n(&strength_state_vars(elem_gid,247), 3, 3); // plastic strain at previous timestep 151-159
    real_t gacumgr = strength_state_vars(elem_gid,256); // accumulated slip shear at previous timestep
    // read local bunge euler angles and generate rotation matrix
    real_t ph = strength_state_vars(elem_gid,257);//*(4.0*atan(1.0))/180.0; 
    real_t th = strength_state_vars(elem_gid,258);//*(4.0*atan(1.0))/180.0; 
    real_t tm = strength_state_vars(elem_gid,259);//*(4.0*atan(1.0))/180.0;
    euler(2, ph, th, tm, ag.pointer());
    // -----------------------------------------------------------------------------
    // END Decompress strength_state_vars
    // CUUUUUUDDAAAAAA
    ...
    // Recompress strength_state_vars (this may become its own function later)
    // // Overwrite stress to evolve forward-euler instead of RK
    // for (int i = 1; i <= 3; i++) {
    //   for (int j = 1; j <= 3; j++) {
    //     stress_n(i,j) = stress(i,j);
    ...
    strength_state_vars(elem_gid,256)=gacumgr;
    // 161-163 calc new euler angles
    euler(1, ph, th, tm, ag.pointer());
    strength_state_vars(elem_gid,257) = ph; 
    strength_state_vars(elem_gid,258) = th; 
    strength_state_vars(elem_gid,259) = tm;
    // END Recompress strength_state_vars
...
} // end namespace EVPStrengthModel