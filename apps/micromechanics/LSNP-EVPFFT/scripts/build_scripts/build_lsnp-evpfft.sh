...
# --------building LSNP-EVPFFT
LSNP_EVPFFT_SOURCE_DIR="$PARENT_DIR/src"
LSNP_EVPFFT_BUILD_DIR="$PARENT_DIR/lsnp-evpfft_${heffte_build_type}_${kokkos_build_type}"
...
# Configure LSNP-EVPFFT using CMake
cmake_options=(
...
# Configure LSNP-EVPFFT
cmake "${cmake_options[@]}" -B "$LSNP_EVPFFT_BUILD_DIR" -S "$LSNP_EVPFFT_SOURCE_DIR"

# Build kokkos
echo "Building LSNP-EVPFFT..."
make -C "$LSNP_EVPFFT_BUILD_DIR" -j"$num_jobs"
