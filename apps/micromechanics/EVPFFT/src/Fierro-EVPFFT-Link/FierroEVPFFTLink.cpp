...
namespace FierroEVPFFTLink
{
  void init_strength_state_vars(
    ...
    {
      printf("Executing FierroEVPFFTLink::init_strength_state_vars ...\n");
      ...
      for (size_t elem_gid = 0; elem_gid < num_elems; elem_gid++) {
        ...
        if (material.host(mat_id).strength_model == STRENGTH_MODEL::evpfft) {
  ...
  void calc_stress (
    ...
    const double time)
    {
      ...
      {
        throw std::runtime_error("EVPFFT not initialized in this element");
      }      
...
} // end namespace FierroEVPFFTLink
