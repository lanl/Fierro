...
void correct_force_tensor(CArrayKokkos <double> &F_c,
                         const size_t stage,
                         const mesh_t &mesh,
        ...
        FOR_ALL(node_gid, 0, mesh.num_nodes,{
            for(int dim = 0; dim < mesh.num_dims; dim++){
               delta_f(node_gid, dim) = -( 1.0 + double(stage) )*M_dot_L2(node_gid, dim)/dt - ( 1.0 + double(stage) )*F_dot_ones(node_gid,dim);
        ...
        FOR_ALL(node_gid, 0, mesh.num_nodes,
                ...
                F_c(node_gid, zone_gid, dim) = delta_f(node_gid, dim)/mesh.num_zones;
