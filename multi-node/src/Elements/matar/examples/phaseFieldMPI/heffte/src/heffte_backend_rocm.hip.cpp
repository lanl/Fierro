...
void cos_pre_pos_processor::pre_forward(hipStream_t stream, int length, precision const input[], precision fft_signal[]){
    dim3 threads( BLK_X, 1 );
    dim3 grid( (length + BLK_X-1)/BLK_X, 1 );
    cos_pre_forward_kernel<<<grid, threads, 0, stream>>>(length, input, fft_signal);
}
template<typename precision>
void cos_pre_pos_processor::post_forward(hipStream_t stream, int length, std::complex<precision> const fft_result[], precision result[]){
    dim3 threads( BLK_X, 1 );
    dim3 grid( (length + BLK_X-1)/BLK_X, 1 );
    cos_post_forward_kernel<<<grid, threads, 0, stream>>>(length, reinterpret_cast<precision const*>(fft_result), result);
}
template<typename precision>
void cos_pre_pos_processor::pre_backward(hipStream_t stream, int length, precision const input[], std::complex<precision> fft_signal[]){
    dim3 threads( BLK_X, 1 );
    dim3 grid( (length + BLK_X-1)/BLK_X, 1 );
    cos_pre_backward_kernel<<<grid, threads, 0, stream>>>(length, input, reinterpret_cast<precision*>(fft_signal));
}
template<typename precision>
void cos_pre_pos_processor::post_backward(hipStream_t stream, int length, precision const fft_result[], precision result[]){
    dim3 threads( BLK_X, 1 );
    dim3 grid( (length + BLK_X-1)/BLK_X, 1 );
    cos_post_backward_kernel<<<grid, threads, 0, stream>>>(length, fft_result, result);
}
template<typename precision>
void sin_pre_pos_processor::pre_forward(hipStream_t stream, int length, precision const input[], precision fft_signal[]){
    dim3 threads( BLK_X, 1 );
    dim3 grid( (length + BLK_X-1)/BLK_X, 1 );
    sin_pre_forward_kernel<<<grid, threads, 0, stream>>>(length, input, fft_signal);
}
template<typename precision>
void sin_pre_pos_processor::post_forward(hipStream_t stream, int length, std::complex<precision> const fft_result[], precision result[]){
    dim3 threads( BLK_X, 1 );
    dim3 grid( (length + BLK_X-1)/BLK_X, 1 );
    sin_post_forward_kernel<<<grid, threads, 0, stream>>>(length, reinterpret_cast<precision const*>(fft_result), result);
}
template<typename precision>
void sin_pre_pos_processor::pre_backward(hipStream_t stream, int length, precision const input[], std::complex<precision> fft_signal[]){
    dim3 threads( BLK_X, 1 );
    dim3 grid( (length + BLK_X-1)/BLK_X, 1 );
    sin_pre_backward_kernel<<<grid, threads, 0, stream>>>(length, input, reinterpret_cast<precision*>(fft_signal));
}
template<typename precision>
void sin_pre_pos_processor::post_backward(hipStream_t stream, int length, precision const fft_result[], precision result[]){
    dim3 threads( BLK_X, 1 );
    dim3 grid( (length + BLK_X-1)/BLK_X, 1 );
...
#define heffte_instantiate_cos(precision) \
    template void cos_pre_pos_processor::pre_forward<precision>(hipStream_t, int, precision const[], precision[]); \
    template void cos_pre_pos_processor::post_forward<precision>(hipStream_t, int,  std::complex<precision> const[], precision[]); \
    template void cos_pre_pos_processor::pre_backward<precision>(hipStream_t, int, precision const[], std::complex<precision>[]); \
    template void cos_pre_pos_processor::post_backward<precision>(hipStream_t, int, precision const[], precision[]); \
    template void sin_pre_pos_processor::pre_forward<precision>(hipStream_t, int, precision const[], precision[]); \
    template void sin_pre_pos_processor::post_forward<precision>(hipStream_t, int,  std::complex<precision> const[], precision[]); \
    template void sin_pre_pos_processor::pre_backward<precision>(hipStream_t, int, precision const[], std::complex<precision>[]); \
    template void sin_pre_pos_processor::post_backward<precision>(hipStream_t, int, precision const[], precision[]); \
