    ...
    KOKKOS_CLASS_LAMBDA(const int k, const int j, const int i,
      ...
      for (int jj = 1; jj <= 3; jj++) {
        for (int ii = 1; ii <= 3; ii++) {
          ...
          scauav_loc(ii,jj) += sg(ii,jj,i,j,k) * wgt;

          if (jphase(i,j,k) == 1) {
            scauav1_loc(ii,jj) += sg(ii,jj,i,j,k) * wgt/wph1;
          }
...
//
  ...
  for (int i = 1; i <= 6; i++) {
    ...
    if (idsim(i) == 0) {
      for (int k = 1; k <= 6; k++) {
        ...
        ddisgradmacro.host(ii,jj) += s0.host(ii,jj,kk,ll) * iscau(k) *
  ...
  for (int jj = 1; jj <= 3; jj++) {
    for (int ii = 1; ii <= 3; ii++) {
      ddisgradmacroacum(ii,jj) += ddisgradmacro.host(ii,jj);
    }
  }
