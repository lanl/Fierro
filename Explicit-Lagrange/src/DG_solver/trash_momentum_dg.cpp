...
// and this line to return to og dg //
    for (int elem_gid = 0; elem_gid < mesh.num_elems(); elem_gid++){
        for(int gauss_lid = 0; gauss_lid < mesh.num_gauss_in_elem(); gauss_lid++){
	    ...
	    if (cycle == 1){
	      // create array of velocity coefficients
	      auto vel_temp = CArray <real_t> (mesh.num_dim(), elem.num_basis());
	      for (int dim = 0; dim < mesh.num_dim(); dim++){
	        for (int basis_id = 0; basis_id < elem.num_basis(); basis_id++){
	          
		  int node_basis_id = elem.vert_node_map(basis_id);
		  int interp_gid = mesh.gauss_in_elem(elem_gid, node_basis_id);
		  vel_temp(dim,interp_gid) = mat_pt.velocity(1,interp_gid, dim);

	        }// end loop over basis_id
	      }// end loop over dim
              
	      // Bernstein-Vandermonde matrix
	      auto BV_mat = CArray <real_t> (elem.num_basis(), elem.num_basis(), mesh.num_dim());
	      for (int j = 0; j < elem.num_basis(); j++){
	        for (int i = 0; i < elem.num_basis(); i++){
		  for (int dim =0 ; dim < mesh.num_dim(); dim++){
		    int node_basis_id = elem.vert_node_map(i);
                    int interp_gid = mesh.gauss_in_elem(elem_gid, node_basis_id);
		    BV_mat(i,j,dim) = ref_elem.ref_nodal_basis(i,j);//bernstein::eval(elem.num_basis()-1, j, node.coords(1,interp_gid,dim));
		  } // end loop over dim
		}// end loop over i
	      }//end loop over j
              
	      // Compute LU inverse of BV_mat
	      ...
	      for (int dim = 0; dim < mesh.num_dim(); dim++){
                auto BV = ViewCArray <real_t> (&BV_mat(0,0,dim), elem.num_basis(), elem.num_basis());

	        auto BV_inv_temp = CArray <real_t> (elem.num_basis(), elem.num_basis());
	        auto BV_inv = ViewCArray <real_t> (&BV_inv_temp(0,0),elem.num_basis(), elem.num_basis());
                LU_invert(BV, index, BV_inv, col, elem.num_basis());

	        auto B_coeffs = CArray <real_t> (elem.num_basis());

	        for (int j = 0; j < elem.num_basis(); j++){
	          for (int i = 0; i < elem.num_basis(); i++){
	            //for (int dimension = 0; dimension < mesh.num_dim(); dimension++){
		      B_coeffs(j) += BV_inv(i,j)*vel_temp(dim,j);
		   // }// end loop over dimension
                ...
                for(int basis_id = 0; basis_id < elem.num_basis(); basis_id++){
                  ...
                  int node_basis_id = elem.vert_node_map(basis_id);
                  int interp_gid = mesh.gauss_in_elem(elem_gid, node_basis_id);
                  interp_vel[dim] += B_coeffs(interp_gid)*ref_elem.ref_nodal_basis(gauss_lid, basis_id);
