...
void input(CArrayKokkos <material_t> &material,
    ...
    graphics_dt_ival  = 0.1;//1.0;//time_final;
    ...
    max_num_state_vars = 8;  // it is a memory block
    ...
    num_bcs = 6;  // =6 for Sedov, Triple, TGV, Sod; =3 Noh3D; 1 for material (box)
    boundary = CArrayKokkos <boundary_t> (num_bcs);  // create boundaries
    
    // --- test problems ---
    test_problem = test::Sedov3D;//test::TaylorGreen;//test::TriplePoint;//test::Noh3D;//test::TaylorAnvil;//test::box;//
    ...
    if (test_problem == test::Sedov3D){
        ...
        RUN({
            ...
            state_vars(0,0) = 5.0/3.0; // 1.4;//  // gamma value
            ...
            mat_fill(1).radius2 = 1.2/16.0;//16.0;//32.0;//30.0;//      // outer radius of fill region
    ...
    if (test_problem == test::TriplePoint){
        ...
        RUN({
            ...
            material(0).num_state_vars = 3;  // actual num_state_vars
            state_vars(0,0) = 1.5; // gamma value
            state_vars(0,1) = 1.0E-14; // minimum sound speed
            state_vars(0,2) = 1.0;     // specific heat

            state_vars(1,0) = 1.4; // gamma value
            state_vars(1,1) = 1.0E-14; // minimum sound speed
            state_vars(1,2) = 1.0;     // specific heat

            state_vars(2,0) = 1.5; // gamma value
            state_vars(2,1) = 1.0E-14; // minimum sound speed
            state_vars(2,2) = 1.0;     // specific heat
    ...
    if (test_problem == test::box){
        ...
        RUN({
            ...
            material(0).eos_model = user_eos_model;//ideal_gas;// EOS model is required
            
            material(0).strength_type =  model::hypo; //model::none; // 
            material(0).strength_model = user_strength_model; //NULL;// 
            ...
            mat_fill(0).sie = 1e-16;               // initial specific internal energy
