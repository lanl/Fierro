...
void input(CArrayKokkos <material_t> &material,
    ...
    max_num_state_vars = 10;  // it is a memory block
    ...
    num_bcs=3;  // =6 for Sedov  =3 for Noh
    boundary = CArrayKokkos <boundary_t> (num_bcs);  // create boundaries
    
    // --- test problems ---
    test_problem = test::NohRZ; //test::TaylorAnvilRZ;
    ...
    if (test_problem == test::SedovRZ){
        ...
        RUN({
            ...
            mat_fill(1).mat_id = 0;              // material id
            mat_fill(1).radius1 = 0.0;           // inner radius of fill region
            mat_fill(1).radius2 = 1.2/50.0;       // outer radius of fill region
            mat_fill(1).den = 1.0;               // initial density
            
            // cylinder
            double vol;
            vol = PI*( pow((mat_fill(1).radius2),2) )*mat_fill(1).radius2;
            
            //vol = 4./3.* PI * ( pow((mat_fill(1).radius2),3) - pow((mat_fill(1).radius1),3) )/2.0;
            mat_fill(1).sie = (0.49339/vol);
    ...
    if (test_problem == test::NohRZ){
        ...
        RUN({
            ...
            // Tag Y plane
            boundary(1).surface = bdy::xy_plane;
            boundary(1).value = 0.0;
            boundary(1).hydro_bc = bdy::reflected;
    ...
    if (test_problem == test::Verney){
        ...
        RUN({
            ...
            material(0).strength_model = user_strength_model_2D;
            ...
            material(0).num_state_vars = 7;  // actual num_state_vars
            // variables are for steel
            state_vars(0,0) = 1.93;  // gamma value for steel
            state_vars(0,1) = 0.6;   // c0
            state_vars(0,2) = 1.49;  // s0 for aluminum
            state_vars(0,3) = 7.9;   // refernce den
            state_vars(0,4) = 0.001; // cs min
            
            state_vars(0,5) = 0.895; // shear modulus of steel
            state_vars(0,6) = 0.05;  // yield strength of steel
            ...
            mat_fill(0).volume = region::global; // fill everywhere
            mat_fill(0).mat_id = 0;              // material id
            mat_fill(0).den = 7.9;               // intial density
            mat_fill(0).sie = 1.e-10;            // intial specific internal energy
            
            mat_fill(0).velocity = init_conds::radial;
            mat_fill(0).speed = 0.1;    // .14*.r_o^2 and r_0 = 8
            // x/r0^2 y/r^2
            ...
            // Tag X plane
            boundary(0).surface = bdy::y_plane; // planes, cylinder, spheres, or a files
            boundary(0).value = 0.0;
            boundary(0).hydro_bc = bdy::reflected;
    ...
    if (test_problem == test::box){
        ...
        RUN({
            ...
            material(0).strength_type =  model::hypo; //model::none; // model::hypo;
            material(0).strength_model = user_strength_model;  // NULL;// user_strength_model;
            
            material(0).q1        = 0.1;       // accoustic coefficient
            material(0).q2        = 1.3333;    // linear slope of UsUp for Riemann solver
            material(0).q1ex      = 0.1;       // accoustic coefficient in expansion
            material(0).q2ex      = 0.0;       // linear slope of UsUp in expansion
            ...
            // statev(2) = s0
            // statev(3) = den_ref
            // statev(4) = shear modulus
            // statev(5) = yield strength
            
            material(0).num_state_vars = 7;  // actual num_state_vars
            // variables are for steel
            // eos
            state_vars(0,0) = 2.0;    // gamma value
            state_vars(0,1) = 0.533;  // C0 steel
            state_vars(0,2) = 1.34;   // s0
            state_vars(0,3) = 2.8027; // refernce den
            state_vars(0,4) = 0.001;  // min soundspeed
            // strength
            state_vars(0,5) = 0.286;  // shear modulus
            state_vars(0,6) = 0.0026; // yield strength
            
            // Global instructions
            mat_fill(0).volume = region::global;   // fill everywhere
            mat_fill(0).mat_id = 0;                // material id
            mat_fill(0).den = 2.8027;              // intial density
            mat_fill(0).sie = 1e-16;               // intial specific internal energy
            ...
            // Tag bottom z plane
            boundary(0).surface = bdy::z_plane; // planes, cylinder, spheres, or a files
            boundary(0).value = 0.0;
            boundary(0).hydro_bc = bdy::reflected;
            
            // Tag top z plane
            //boundary(1).surface = bdy::z_plane; // planes, cylinder, spheres, or a files
            //boundary(1).value = 2.54; //1.2;
            //boundary(1).hydro_bc = bdy::velocity;
            //boundary(1).hydro_bc_vel_0 = -0.0001249;  // -0.001
            //boundary(1).hydro_bc_vel_1 = 0.0035;  // cm/us
            //boundary(1).hydro_bc_vel_t_start = 0;
            //boundary(1).hydro_bc_vel_t_end = 1012; // mu seconds
            
            // Tag X plane
            //boundary(0).surface = bdy::z_plane; // planes, cylinder, spheres, or a files
            //boundary(0).value = 0.0;
            //boundary(0).hydro_bc = bdy::velocity;
            //boundary(0).u = 0.0;
            //boundary(0).v = 0.0;
            //boundary(0).w = 1.0;
    ...
    if (test_problem == test::TaylorAnvilRZ){
        ...
        RUN({
            ...
            material(0).strength_type =  model::hypo; //model::none; // model::hypo;
            material(0).strength_model = user_strength_model_2D;  // NULL;// user_strength_model;
            ...
            mat_fill(0).sie = 1e-16;               // intial specific internal energy
                ...
                material(0).strength_type =  model::hypo; //model::none; // model::hypo;
                material(0).strength_model = user_strength_model_2D; //user_strength_model_2D;  // NULL;// user_strength_model;
