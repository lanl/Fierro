    ...
    FOR_ALL_CLASS (elem_gid, 0, rnum_elem, {
        ...
        for (size_t node_lid = 0; node_lid < num_nodes_in_elem; node_lid++) {
            ...
            for (int dim = 0; dim < num_dims; dim++){
                ...
                //test clause
                //corner_force(corner_gid, dim) = -0.00001*node_vel(1, node_gid, dim) - 0.0001*node_coords(1, node_gid, dim) + 0.0001*relative_element_densities(elem_gid);  
                //corner_force(corner_gid, dim) = 0.0001*relative_element_densities(elem_gid);
