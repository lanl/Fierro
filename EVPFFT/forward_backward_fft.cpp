...
void EVPFFT::forward_fft()
{
  ...
  for (int ii = 1; ii <= 3; ii++) {
    for (int jj = 1; jj <= 3; jj++) {
      ...
      // perform forward FFT
      fft.forward(data.host_pointer(), data_cmplx.host_pointer());
      data_cmplx.update_device();
#else
      // perform forward FFT
      fft.forward(data.device_pointer(), data_cmplx.device_pointer());
#endif
...
void EVPFFT::backward_fft()
{
  ...
  for (int ii = 1; ii <= 3; ii++) {
    for (int jj = 1; jj <= 3; jj++) {
      ...
      fft.backward(data_cmplx.host_pointer(), data.host_pointer());
      data.update_device();
#else
      // perform backward FFT
      fft.backward(data_cmplx.device_pointer(), data.device_pointer());
#endif
      ...
      FOR_ALL_CLASS(k, 1, npts3+1,
                    j, 1, npts2+1,
                    i, 1, npts1+1, {
        work(ii,jj,i,j,k) = data(i,j,k) * wgt;
      }); // end FOR_ALL_CLASS
      Kokkos::fence();
