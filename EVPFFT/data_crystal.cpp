...
void EVPFFT::data_crystal(int iph, const std::string & filecryspl)
{
  ...
  if (nmodes(iph) > NMODMX) {
    printf("nmodes IN PHASE %d IS %d\n", iph, nmodes(iph));
    printf("CHANGE PARAMETER NMODMX IN fft.dim\n");
    exit(1);
  }
  ...
  for (int nm = 1; nm <= nmodesx; nm++) {
      ...
      if (ntwmod(iph) > NTWMMX) {
        printf("NTWMOD IN PHASE %d IS %d\n", iph, ntwmod(iph));
        printf("CHANGE PARAMETER NTWMMX IN VPSC.DIM\n");
        exit(1);
      }
    ...
    //=====================================================================
      ...
      for (int js = 1; js <= nsm(kount,iph); js++) {
        ...
        if (nsyst.host(iph) > NSYSMX) {
          printf("NSYST IN PHASE %d IS %d\n", iph, nsyst.host(iph));
          printf("CHANGE PARAMETER NSYSMX IN VPSC.DIM\n");
          exit(1);
        }
